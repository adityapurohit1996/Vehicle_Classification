...
classes = (
    'Uknown', 'Compacts', 'Sedans', 'SUVs', 'Coupes',
...
for k, b in enumerate(bbox):
    ...
    ############## This is for making bounding boxes [S] ##############
    # pdb.set_trace()
    x_coords = vert_2D[0,:]
    y_coords = vert_2D[1,:]
    z_coords = vert_3D[2,:]
    if max(z_coords) < 50:
        ...
        ax1.plot([min_x, max_x], [max_y, max_y], color=clr)
        ax1.plot([max_x, max_x], [max_y, min_y], color=clr)
        ax1.plot([max_x, min_x], [min_y, min_y], color=clr)
        ax1.plot([min_x, min_x], [min_y, max_y], color=clr)
    ############## This is for making bounding boxes [E] ##############
    c = classes[int(b[9])]
